...
if st.session_state.final_prompt_chatbot:
    ...
    # Email 및 Password 입력
    email = st.text_input("📧 Email (필수) 학생의 생성결과물을 받아볼 수 있습니다.", value=st.session_state.get('email_chatbot', '')).strip()
    password = st.text_input("🔒 Password (필수) 저장한 프롬프트를 조회, 삭제할 수 있습니다.", value=st.session_state.get('password_chatbot', ''), type="password").strip()
